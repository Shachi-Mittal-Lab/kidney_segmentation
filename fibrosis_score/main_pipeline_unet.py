# Repo Tools
from fibrosis_score.zarr_utils import ndpi_to_zarr, omniseg_to_zarr, svs_to_zarr
...
def run_full_pipeline(
    ...
    s1_array  = open_ds(zarr_path / "raw" / "s1")
    ...
    # prepare nuclei mask (20x)
    nuclei_mask_20x = prepare_mask(zarr_path, s1_array, "nuclei_mask_20x")
    # prepare histological segmentation mask locations in zarr (40x)
    tubule_mask, vessel_mask, nuclei_mask = prepare_seg_masks(zarr_path, s0_array, "40x")
    ...
    # calculate size affected by padding of the 1056 x 1056 block
    padding_affected_shape = Coordinate(208,208)
    padding_affected_size = padding_affected_shape * s0_array.voxel_size  # size in nm

    print("Predicting Nuclei with U-Net")
    #### Use U-net to predict nuclei at 40x ####
    # load model
    ...
    # predict
    model_prediction_lsds(nuclei_mask_20x, s1_array, patch_size_final, padding_affected_size, model, binary_head, device, "Nuclei ID")
    ...
    # blockwise upsample from 20x to 40x
    upsampling_factor = nuclei_mask_20x.voxel_size / nuclei_mask.voxel_size
    upsample(nuclei_mask_20x, nuclei_mask, upsampling_factor, s1_array, s0_array)
    ...
    id_tbm(tubule_mask, nuclei_mask, tbm_mask, s0_array)
