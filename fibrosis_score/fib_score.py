...
# convert ndpi to zarr array including* 1e7 levels 40x, 20x, 10x, 5x
for i in range(0, 4):
    voxel_size = Coordinate(
        int(1 / 44114.0 * 1e7) * 2**i, int(1 / 44114.0 * 1e7) * 2**i
    )
    # dask_array, x_res, y_res, units = openndpi(ndpi_path,i)
    float_arr = open_ds(zarr_float_path / "raw" / f"s{i}")
    units = ("nm", "nm")
    # voxel_size = (1/x_res * 2 ** i , 1/y_res * 2 ** i)
    # raw = prepare_ds( zarr_path / "raw" / f"s{i}", dask_array.shape, offset, voxel_size, axis_names, units, dtype=np.uint8)
    # raw = prepare_ds( zarr_path / "raw" / f"s{i}", float_arr.shape, float_arr.offset, float_arr.voxel_size, float_arr.axis_names, float_arr.units, dtype=np.uint8)
    # store_rgb = zarr.open(zarr_path / "raw" / f"s{i}")
    # # dask.array.store(dask_array, store_rgb)
    # dask.array.store(float_arr.data, store_rgb)
...
# create integral mask of foreground mask
...
)  # 90% of voxels are tissue

# make grid on s3
patch_spacing = Coordinate(100, 100)

# very strange... why we give y shape then x shape... but it work
