...
def run_full_pipeline(
    ...
    del model 
    # load model
    #model = torch.load("model_unet_dataset8_vessel0wscheduler_200.pt", weights_only=False)
    # predict
    #model_prediction(vessel_mask_10x, s2_array, patch_size_final, model, device, "Vessel ID")

    model = torch.load("model_unet_dataset10_vessel6_LSDs_700.pt", weights_only=False)
    binary_head = torch.load("binaryhead_unet_dataset10_vessel6_LSDs_700.pt", weights_only=False)
    # predict
    print_gpu_usage(device)
    ...
    vessel_mask_10x._source_data[:] = remove_small_objects(
        vessel_mask_10x._source_data[:].astype(bool), min_size=3000
    )
