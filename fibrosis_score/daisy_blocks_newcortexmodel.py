...
def model_prediction_lsds(
    ...
    def process_block(block: daisy.Block):
        # redefine read slices to avoid negative indexes
        actual_read_roi = block.read_roi.intersect(s2_array.roi)
        inslices = s2_array._Array__slices(actual_read_roi)
        actual_read_data = s2_array[inslices]
        read_block = Array(np.zeros(tuple(block.read_roi.shape / s2_array.voxel_size) + (s2_array.shape[2],), dtype=s2_array.dtype), offset=block.read_roi.offset, voxel_size=s2_array.voxel_size, axis_names=s2_array.axis_names, types=s2_array.types, units=s2_array.units)
        read_block_inslices = read_block._Array__slices(actual_read_roi)
        read_block[read_block_inslices] = actual_read_data
        img = Image.fromarray(read_block[:])
        img_shape = img.size
    ...
    pred_task = daisy.Task(
        task,
        # read in padding
        total_roi=s2_array.roi.grow(padding_affected_size, padding_affected_size),
        read_roi=Roi(-padding_affected_size, patch_size_final),  # (offset, shape)
        write_roi=Roi((0,0), patch_size_final -  padding_affected_size[0] * 2),
        read_write_conflict=False,
        num_workers=2,
        process_function=process_block,
    )
...
# blockwise mask multiplications
    def fibfilter_block(block: daisy.Block):
        ...
        filtered_fib = remove_small_objects(labeled, max_size=100) # was 2500
